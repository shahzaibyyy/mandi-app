...
/// Same market-style layout as [ReceiptTemplateV4], with PST outside the
/// total dash block and date+time on one line under تاریخ.
class ReceiptTemplateV5 implements ReceiptTemplate {
  static const _body = 16.0;
  static const _company = 17.0;
  static const _table = 15.0;
  static const _summary = 16.0;
  static const _totalValue = 18.0;
  static const _paid = 20.0;
  ...
  Future<Uint8List> buildPng({
    ...
    y = _center(canvas, company, y, widthPx, size: _company, bold: true, gap: 4);
    ...
    y = _kvPair(
      ...
      leftValue: AppConstants.contractorDisplayName(receipt.contractorName),
    );

    final dateTime =
        '${_dateOnly.format(receipt.createdAt)} ${_timeOnly.format(receipt.createdAt)}';
    y = _kvPair(
      canvas,
      y,
      widthPx,
  ...
  double _tableRow(
    ...
    var x = pad;
    _cell(canvas, amount, x, y, amountW, TextAlign.left, bold: bold, ltr: true);
    x += amountW;
    _cell(canvas, rate, x, y, rateW, TextAlign.center, bold: bold, ltr: true);
    x += rateW;
    _cell(canvas, qty, x, y, qtyW, TextAlign.center, bold: bold, ltr: true);
    x += qtyW;
    final namePainter = _cell(
      canvas,
      name,
      x,
      y,
